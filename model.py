...
class HSDEModel(scviMixin, dipMixin, betatcMixin, infoMixin):
    ...
    def take_time(self, state):
        ...
        state = torch.tensor(state, dtype=torch.float32).to(self.device)
        # Use encoder to get time
        output = self.nn.encoder(state)
        # Handle different encoder return signatures if necessary
        t = output[-1] 
        return t.cpu().numpy()
    
    @torch.no_grad()
    def take_grad(self, state):
        """Extract SDE drift field (Raw)."""
        if not self.use_sde:
            raise ValueError("take_grad() requires use_sde=True")
        
        state = torch.tensor(state, dtype=torch.float32).to(self.device)
        
        # 1. Get latent state and time
        q_z, q_m, q_s, n, t = self.nn.encoder(state)
        
        # 2. Get Raw Drift
        # We want the pure directional signal learned by the SDE
