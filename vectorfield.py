...
class VectorFieldMixin:
    ...
    Supports both batch-based (MLP/Transformer) and graph-based encoders.
    When using a graph encoder, edge_index and edge_weight are passed through
    to `take_grad()`.
    """
    ...
    def get_vfres(
        ...
        # Compute raw drift (pass graph info if available)
        edge_index = getattr(self, "edge_index", None)
        edge_weight = getattr(self, "edge_weight", None)
        grads = self.take_grad(self.X_norm, edge_index=edge_index, edge_weight=edge_weight)
    ...
    def _auto_detect_direction(
        ...
        Checks alignment (drift→future) and magnitude trend (late/early).
